...
def plot_refined_mesh_with_boundaries(nodes, elements, boundary_groups, periodic_pairs, n=1):
    """Plot the mesh with boundary groups colored differently."""
    fig, ax = plt.subplots(1, 1, figsize=(14, 6))
    ...
    ax.set_xlabel('x')
    ax.set_ylabel('y')
    ax.set_title('Mesh with Boundary Groups (Refinement Level {})'.format(n))
    ax.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig(f"data/blade_refined_mesh_boundaries{n}.png", dpi=600)
    print(f"Saved data/blade_refined_mesh_boundaries{n}.png")
    ...
    if periodic_pairs:
        ...
        plt.tight_layout()
        plt.savefig(f"data/blade_refined_mesh_periodic{n}.png", dpi=600)
        print(f"Saved data/blade_refined_mesh_periodic{n}.png")
...
def plot_solution(Mesh, field, title='Solution', cmap='viridis', show_mesh=False): # AI GENERATED - REDO LATER IF WANTED
    """
    Plot a solution field on the triangular mesh.
    
    Parameters:
    -----------
    Mesh : dict
        Mesh dictionary with 'V' (nodes) and 'E' (elements)
    field : ndarray
        Field values, one per element [nElem]
    title : str, optional
        Plot title
    cmap : str, optional
    ...
    # Use tripcolor for element-centered data
    # We need to compute element centroids and interpolate to nodes for tripcolor
    # OR use tricontourf for smoother visualization
    
    # Option 1: Simple approach - assign element values to nodes (average of adjacent elements)
    nNodes = V.shape[0]
    nElem = E.shape[0]
    node_values = np.zeros(nNodes)
    node_count = np.zeros(nNodes)
    
    for i in range(nElem):
        for j in range(3):
            node_idx = E[i, j]
            node_values[node_idx] += field[i]
            node_count[node_idx] += 1
    
    # Average
    node_values = node_values / np.maximum(node_count, 1)
    
    # Create triangulation
    import matplotlib.tri as tri
    triang = tri.Triangulation(V[:, 0], V[:, 1], E)
    
    # Plot with tricontourf for smooth contours
    levels = 20
    tcf = ax.tricontourf(triang, node_values, levels=levels, cmap=cmap)
    
    # Add colorbar
    cbar = fig.colorbar(tcf, ax=ax)
    cbar.set_label(title)
    
    # Optionally show mesh
    if show_mesh:
