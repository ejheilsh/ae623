...
#-----------------------------------------------------------
def plotmesh(Mesh, fname: str, savefig=True):
    V = Mesh['V']; E2N = Mesh['E']; BE = Mesh['BE']
    fig, ax = plt.subplots()
    ax.triplot(V[:,0], V[:,1], E2N, 'k-', linewidth=0.5)
    for i in range(BE.shape[0]):
        ax.plot(V[BE[i,0:2], 0], V[BE[i,0:2], 1], '-', linewidth=1, color='black')
