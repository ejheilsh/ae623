...
def update_state_parallel(U, R, dtA, CFL):
    """
    Parallel explicit Euler time step update -- we now want to update unsteadily:: from Fidowski::
    "When solving an unsteady problem time-accurately, every cell takes the same time step,
    which is set to be the smallest (most restrictive) over all of the cells"
    """
    Ne = U.shape[0]
    dt_smallest = np.min(2 * CFL / (dtA))
    for i in prange(Ne):
        for k in range(4):
            U[i, k] = U[i, k] - dt_smallest * R[i, k]
    return dt_smallest  # Return time step for monitoring
    

def solve(Mesh, flow_params, niter, n_save, U0=None):
    """
    Euler solver
    ...
    n_save : int
        Save solution every n_save iterations for diagnostics
    cum_time : float
        Cumulative time from previous runs (for unsteady monitoring)
    U0 : ndarray, optional
    ...
    # Initialize state -- start from the steady solution
    if U0 is None:
    ...
    Rhist = []
    cum_time = [0.0]
    Uhist = [U.copy()]  # Store initial condition for diagnostics
    current_time = 0.0  # Track cumulative physical time
    
    for iiter in range(niter):
        # Calculate residual (spatial flux divergence = time derivative)
        R, dtA = calc_res(Mesh, Normals, flow_params, U, current_time)        
        
        # Update the state (explicit Euler time step)
        dt_used = update_state_parallel(U, R, dtA, CFL)
        current_time += dt_used  # Advance physical time
        
        # For unsteady: monitor solution change, not spatial residual
        if len(Uhist) > 0:
            dU = U - Uhist[-1]  # Change from previous time step
            dU_norm = np.linalg.norm(dU)
            R_norm = np.linalg.norm(R)  # Time derivative magnitude
        else:
            dU_norm = 0.0
            R_norm = np.linalg.norm(R)
        
        # Check convergence every iteration for unsteady monitoring
        if iiter % 1 == 0:  
            Rhist.append(R_norm)  
            
            print(f'iiter = {iiter:5d}, ||dU/dt|| = {R_norm:10.4e}, ||ΔU|| = {dU_norm:10.4e}, dt = {dt_used:10.4e}')
            ...
            # For unsteady: convergence means solution stops changing
            # (not that spatial residual goes to zero)
            if dU_norm < Rtol:
                print('Reached steady state (solution stopped changing)!')
                break
        if iiter % n_save == 0:
            Uhist.append(U.copy())  
            cum_time.append(current_time)  
    return U, Rhist, Uhist, cum_time
...
def calc_res_interior_parallel(IE, U, inormal, ilength, gamma, R, dtA):
    ...
    Uses a 2-pass approach:
      1) parallel edge flux compute
      2) serial accumulation into R and dtA
    """
    Ni = IE.shape[0]
    Fedge = np.empty((Ni, 4), dtype=np.float64)
    smag_edge = np.empty(Ni, dtype=np.float64)

    compute_interior_edge_fluxes(IE, U, inormal, gamma, Fedge, smag_edge)
    accumulate_interior_edge_fluxes(IE, ilength, Fedge, smag_edge, R, dtA)


@njit(parallel=True)
def compute_interior_edge_fluxes(IE, U, inormal, gamma, Fedge, smag_edge):
    Ni = IE.shape[0]
    for i in prange(Ni):
        eL = IE[i, 2]
        eR = IE[i, 3]
...
@njit
def accumulate_interior_edge_fluxes(IE, ilength, Fedge, smag_edge, R, dtA):
    Ni = IE.shape[0]
    for i in range(Ni):
        eL = IE[i, 2]
        eR = IE[i, 3]
...
def calc_res_boundary_serial(BE, U, bnormal, blength, gamma, R, dtA, 
                              ...
                              rho0, p0, alpha, pout, periodic_map_array, V, current_time):
    """
    ...
    for i in range(Nb):
        ...
        if ib == BN_inflow:
            # Calculate y-position at boundary edge (midpoint of edge)
            n1, n2 = BE[i, 0], BE[i, 1]
            y_pos = 0.5 * (V[n1, 1] + V[n2, 1])
            # Subsonic inflow: specify stagnation conditions with unsteady wake
            F, smag = subsonic_inflow_bc(U[eL, :], bnormal[i, :], rho0, p0, alpha, gamma, y_pos, current_time)
        elif ib == BN_outflow:
...
def calc_res(Mesh, Normals, flow_params, U, current_time):
    """
    ...
    calc_res_boundary_serial(BE, U, Normals['bnormal'], Normals['blength'], gamma, R, dtA,
                             ...
                             periodic_map_array, V, current_time)
...
@njit
def subsonic_inflow_bc(UL, n, rho0, p0, alpha, gamma, y, t):
    """
    ...
    where fwake = 0.1, δ = 0.1, ∆y = 18mm, yrot is the y position on the rotor (i.e. the y position in your
    domain), Vrot is the rotor speed, and b·c is the greatest-integer (floor) function. Set the rotor speed
    to the stagnation speed of sound at the inflow, Vrot = a0,inflow.
    """
    # interior primitives
...
if __name__ == '__main__':
    ...
    # Load U0 from steady solution file
    U0 = np.loadtxt('data/steady/coarse/solution.txt')
    U, Rhist, Uhist, cum_time = solve(Mesh, flow_params, niter, n_save_interval, U0=U0)
    
    print("="*60)
    if len(Rhist) > 0:
        print(f"Final ||dU/dt|| = {Rhist[-1]:10.4e} (time derivative magnitude)")
        print(f"Time steps completed: {len(Rhist)}")
        if len(Uhist) > 1:
            final_change = np.linalg.norm(Uhist[-1] - Uhist[-2])
            print(f"Final solution change ||ΔU|| = {final_change:10.4e}")
    
    # Save solution to an animation file of unsteady history
    for i, U_snapshot in enumerate(Uhist):
        output_file = f'data/unsteady/coarse/solution_{cum_time[i]:.2f}_{i:04d}.txt'
        np.savetxt(output_file, U_snapshot)
        print(f"Saved snapshot {i} to {output_file}")
